...
import com.taobao.weex.utils.BinaryDataUtils;
...
public class WebSocketModule extends WXSDKEngine.DestroyableModule {
  ...
  private IWebSocketAdapter.EventListener eventListener = new IWebSocketAdapter.EventListener() {
    ...
    public void onMessage(String data) {
      ...
      WXLogUtils.e("WebSocketModule", "IWebSocketAdapter#onMessage(String data) has been deprecated!");
    }

    @Override
    public void onMessage(int type, Object data) {
      if (onMessage != null) {
        Map<String, Object> msg = new HashMap<>(1);
        if (type == IWebSocketAdapter.PAYLOAD_TYPE_TEXT && data instanceof String) {
          msg.put(KEY_DATA, data);
        } else if (type == IWebSocketAdapter.PAYLOAD_TYPE_BINARY && data instanceof byte[]) {
          msg.put(KEY_DATA, BinaryDataUtils.pack((byte[]) data));
        }
        onMessage.invokeAndKeepAlive(msg);
      }
    }
    ...
    if (!reportErrorIfNoAdapter()) {
      try {
        ...
        if (dataObject instanceof JSONObject) {
          String type = ((JSONObject) dataObject).getString("@type");
          if ("binary".equals(type)) {
            byte[] payload = BinaryDataUtils.unpack((JSONObject) dataObject);
            webSocketAdapter.send(IWebSocketAdapter.PAYLOAD_TYPE_BINARY, payload);
          }
        } else {
          webSocketAdapter.send(IWebSocketAdapter.PAYLOAD_TYPE_TEXT, dataObject.toString());
        }
      } catch (Exception e) {
        e.printStackTrace();
      }
